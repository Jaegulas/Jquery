...
//-----------------------------------------------------------
   ...
   */
	...
	/*
	 3. autoPlay함수 정의
	    역할: 4초 간격으로 호출되어 [1] ~ [4] 순차적으로 강제로 클릭되어 자동으로 배너 이미지영역<dd>을 보이도록 하는 기능
	*/
	let btnNum = 0;
	
	function autoPlay(){
		
		btnNum++;  //일정한 시긴간격으로 실행문이 실행될때 마다 변수 btnNum의 값이 1씩 증가 되게 하자
		
		//이때 btnNum변수의 값이 4이상 되는 순간 0으로 변경해서  [1] <a>를 강제 클릭할수 있도록 btnNum변수값 0으로 변경
		if(btnNum >= 4) btnNum = 0;
		
		//4초 간격으로 [1] [2] [3] [4] 를 순차적으로 강제 클릭되게 하자.
		$("#roll_banner_wrap dt a").eq(btnNum).trigger("click");
		
		//4초 간격으로 재귀함수 호출(자기 자신의 autoPlay함수 재호출)
	    auto1 = window.setTimeout(autoPlay, 4000);		
			
	}
	
	//최초 한번은 3초 휴식 후 단 한번만 autoPlay함수 호출  
	var auto1 = window.setTimeout( autoPlay, 3000);
	
		
	// 4. 재생 버튼 ▶ <a>를 선택해서 가져와 click이벤트 등록
	$(".playBtn").on("click",function(){
		
		/*  사이트 방문자가 재생 버튼 ▶ 을 여러번 클릭하게 되면 
			auto1변수에 있는 setTimeout()메소드들이 쌓여 있어 문제가 될수 있음
			setTimeout()메소드를 clearTimeout()메소드로 제거한후  setTimeout()메소드 호출구문의 실행을 해야합니다.
		*/
		window.clearTimeout(auto1); //auto1변수에 저장된 setTimeout()메소드 제거 		
		auto1 = window.setTimeout(autoPlay,1000); //1초 지연 후 autoPlay함수 호출
		...
		return false;  // click 이벤트가 발생한 <a>의 href속성의 기본이벤트 차단 
	});
	
	//5. 정지 버튼 ■ <a>영역을 클릭했을때 일정한 시간간격으로 autoPlay함수가 실행되는 것을 제거 하고
	//   정지 버튼 ■ 이미지를 분홍색 이미지로 변경해서 보여주자
	$(".stopBtn").on("click", function(){
		
		//auto1변수에 저장된 setTimeout(autoPlay, 3000) 메소드 호출 구문 제거 
		window.clearTimeout(auto1);
		
		//정지 버튼 ■  회색 이미지를   정지버튼 ■ 분혹생 이미지로 변경해서 보여주자
		$("img",this).attr("src","images/pop_btn_stop_on.gif");
		
		//재생 버튼 ▶ 분홍색 이미지를   재생 버튼 ▶ 회색 이미지로 변경해서 보여주자
 ...
 //-----------------------------------------------------------
   ...
   */
	...
	//mouseover(마우스를 올리는 동작),   focus(포커스를 생성하는 동작),   click(클릭하는 동작) 들을 다중 등록
	$("#tabmenu dt a").on("mouseover focus click", function(){
		
		//먼저 현재 노출(:visible)되어 있는 <dd>태그를 선택하여 
		//게시물 리스트영역<dd>들을 숨김(.hide())
		...
		return false; //<a>의 href속성 기본이벤트 차단
